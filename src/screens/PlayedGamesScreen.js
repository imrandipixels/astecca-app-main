...
export default ({navigation}) => {
  ...
  const [teamPlayedGames, setTeamPlayerGames] = useState(null);
  ...
  const getTeamPlayedGames = async () => {
    try {
      ...
      if (response.status === 200) {
        if (response.data[0]) {
          setTeamPlayerGames(response.data);
        }
      }
    } catch (error) {}
  };
  ...
  const renderTeamGamesList = () => {
    if (teamPlayedGames !== null) {
      return (
        <FlatList
          data={teamPlayedGames}
          renderItem={({item}) => (
            <TeamGameCard
              opponentTeam={item.opponentTeam}
              challengerTeam={item.challengerTeam}
              date={item.date}
